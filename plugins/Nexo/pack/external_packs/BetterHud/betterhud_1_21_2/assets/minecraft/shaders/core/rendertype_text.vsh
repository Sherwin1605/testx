...
#else
uniform mat4 ProjMat;uniform mat4 ModelViewMat;uniform int FogShape;out float vertexDistance;
#endif
in vec3 Position;in vec4 Color;in vec2 UV0;in ivec2 UV2;uniform sampler2D Sampler0;uniform sampler2D Sampler2;uniform vec2 ScreenSize;uniform float GameTime;uniform vec3 ChunkOffset;out vec4 vertexColor;out vec2 texCoord0;out float applyColor;bool range(float t, float m1, float m2) {return t >= m1 && t <= m2;}bool range(vec2 t, vec2 m1, vec2 m2) {return range(t.x, m1.x, m2.x) && range(t.y, m1.y, m2.y);}bool range(vec3 t, vec3 m1, vec3 m2) {return range(t.x, m1.x, m2.x) && range(t.y, m1.y, m2.y) && range(t.z, m1.z, m2.z);}bool checkElement(float z) {if (z == 0) return true; else if (z == 1000) return true; else if (z == -90) return true; else if (z == 2800) return true; return false;}bool checkExp(float z) {if (z == 0) return true; else if (z == 600) return true; return false;}float fogDistance(vec3 pos, int shape) {if (shape == 0) {return length(pos);} else {float distXZ = length(pos.xz);float distY = abs(pos.y);return max(distXZ, distY);}}void main() {vec3 pos = Position;vec2 ui = ceil(2 / vec2(ProjMat[0][0], -ProjMat[1][1]));vec2 uiScreen = ui / ScreenSize;vec3 color = Color.xyz;applyColor = 0;vertexColor = Color * texelFetch(Sampler2, UV2 / 16, 0);if (pos.y >= ui.y && ProjMat[3].x == -1) {int bit = int(pos.y) >> HEIGHT_BIT;if (((bit >> MAX_BIT) & 1) == 1) {int id = bit - (1 << MAX_BIT);pos.x -= 0.5 * ui.x;pos.y -= (bit << HEIGHT_BIT) + ADD_OFFSET + DEFAULT_OFFSET;float xGui = 0;float yGui = 0;float layer = 0;float opacity = 1;bool outline = false;int property = 0;switch (id) {case 1:break;case 2:layer = 1;break;case 3:layer = 2;outline = true;break;case 4:layer = 2;break;case 5:xGui = ui.x * 50.0 / 100.0;layer = -1000;break;case 6:xGui = ui.x * 50.0 / 100.0;outline = true;break;case 7:xGui = ui.x * 50.0 / 100.0;break;case 8:xGui = ui.x * 50.0 / 100.0;layer = 1;break;case 9:xGui = ui.x * 50.0 / 100.0;layer = 2;break;case 10:xGui = ui.x * 50.0 / 100.0;layer = 3;outline = true;break;case 11:xGui = ui.x * 50.0 / 100.0;yGui = ui.y * 100.0 / 100.0;break;case 12:xGui = ui.x * 50.0 / 100.0;yGui = ui.y * 100.0 / 100.0;layer = 1;break;case 13:xGui = ui.x * 50.0 / 100.0;yGui = ui.y * 100.0 / 100.0;layer = 3;outline = true;break;case 14:xGui = ui.x * 100.0 / 100.0;layer = -1000;break;case 15:xGui = ui.x * 100.0 / 100.0;break;case 16:xGui = ui.x * 100.0 / 100.0;yGui = ui.y * 100.0 / 100.0;layer = -1000;break;case 17:xGui = ui.x * 100.0 / 100.0;yGui = ui.y * 100.0 / 100.0;outline = true;break;case 18:xGui = ui.x * 100.0 / 100.0;yGui = ui.y * 100.0 / 100.0;layer = 1;break;case 19:xGui = ui.x * 100.0 / 100.0;yGui = ui.y * 100.0 / 100.0;layer = 2;break;case 20:xGui = ui.x * 100.0 / 100.0;yGui = ui.y * 100.0 / 100.0;layer = 3;break;}
#if SHADER_VERSION < 2
